...
if(fps>30&&insidecamera)
{
	...
	for(i=0; i<image_xscale; i++){
	    for(j=0; j<image_yscale; j++){
			...
			{
				 draw_sprite_ext(sprite_index, index[iy], (ecs+(i*sprite_get_width(sprite_index)))-16, why+(j*sprite_get_height(sprite_index)),0.5,1,0,c_white,1);
			}
			else if(why+(j*sprite_get_height(sprite_index))>=bbox_bottom)
			{
				 draw_sprite_ext(sprite_index, index[iy], ecs+(i*sprite_get_width(sprite_index)), (why+(j*sprite_get_height(sprite_index)))-16,1,0.5,0,c_white,1);
			}
			else
			{
				 draw_sprite(sprite_index, index[iy], ecs+(i*sprite_get_width(sprite_index)), why+(j*sprite_get_height(sprite_index)));
			}
			iy+=1
	    }
	}
